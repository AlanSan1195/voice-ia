...
export function promptFor(payload: AiPayload): string {
  ...
  if (payload.kind === "turnEvaluation")
    return [
      ...
      "When there is a concrete, evidence-backed coaching observation, add coaching with one optional language correction and one interview coaching item. Omit coaching when no safe observation exists. language.original and interview.evidence, when present, must be exact, unique substrings copied from the candidate answer; never invent or paraphrase evidence. language.original/replacement and evidence are English answer fragments. language.why, interview.technique, interview.action, and interview.miniChallenge must be concise Spanish. interview.skill must be technical, relevance, or structure. The miniChallenge is optional practice and must not introduce a new score or invented facts.",
      "Return exactly: {englishScore,professionalEnglishScore,technicalScore,relevanceScore,structureScore,clarityScore,observedEnglishLevel,feedback,strengths:string[],priorityImprovement,correctedAnswer,nextLevelAnswer,coaching?:{language?:{original,replacement,why},interview:{skill,evidence?,technique,action,miniChallenge}}}. strengths must contain 1 or 2 concise items. All six scores must be numbers from 1 to 10. Feedback and coaching explanations must be in Spanish; correctedAnswer and nextLevelAnswer must be in English.",
